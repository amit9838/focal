...
export const App: React.FC = () => {
    ...
    const { shapeType, setShapeType } = useAppStore();
    ...
    // Initialize controller (tools are registered internally)
    useEffect(() => {
    ...
    // Poll drawing mode (temporary – can be replaced with EventBus later)
    useEffect(() => {
    ...
    useEffect(() => {
        if (selectedIds.length > 0) {
            ...
            if (firstShape) {
                ...
                if (fillMatch) {
                    const r = parseInt(fillMatch[1]), g = parseInt(fillMatch[2]), b = parseInt(fillMatch[3]);
                ...
                if (strokeMatch) {
                    const r = parseInt(strokeMatch[1]), g = parseInt(strokeMatch[2]), b = parseInt(strokeMatch[3]);
    ...
    const handleDraw = () => controllerRef.current?.startDrawing();
    const handleCancelDraw = () => controllerRef.current?.cancelDrawing();
    ...
    const handleFillColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        controllerRef.current?.setSelectedFillColor(hexToRgba(newColor, fillOpacity));
    };
    const handleFillOpacityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const newOpacity = parseFloat(e.target.value);
        setFillOpacity(newOpacity);
        controllerRef.current?.setSelectedFillColor(hexToRgba(fillColor, newOpacity));
    };
    const handleStrokeColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const newColor = e.target.value;
        setStrokeColor(newColor);
        controllerRef.current?.setSelectedStrokeColor(hexToRgba(newColor, strokeOpacity));
    };
    const handleStrokeOpacityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const newOpacity = parseFloat(e.target.value);
        setStrokeOpacity(newOpacity);
        controllerRef.current?.setSelectedStrokeColor(hexToRgba(strokeColor, newOpacity));
    };
        ...
        <div className="min-h-screen bg-[#f3f1eb] font-sans flex flex-col items-center">
            ...
            <div className="mt-4 mb-2 bg-white rounded-xl shadow-sm border border-gray-100 px-4 py-2 flex flex-wrap items-center justify-center gap-3">
                <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-1">
                    ...
                    <select value={shapeType} onChange={handleShapeTypeChange} className="bg-white border border-gray-200 rounded-md px-2 py-1 text-sm cursor-pointer">
                ...
                {isDrawingMode && (
                    <button onClick={handleCancelDraw} className="px-4 py-1.5 rounded-lg text-sm font-medium border border-gray-300 text-gray-600 hover:bg-gray-50">
                        Cancel
                    </button>
                )}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input type="color" value={fillColor} onChange={handleFillColorChange} disabled={selectedIds.length === 0} className="w-7 h-7 rounded border border-gray-300 cursor-pointer disabled:opacity-50" />
                    <input type="range" min="0" max="1" step="0.01" value={fillOpacity} onChange={handleFillOpacityChange} disabled={selectedIds.length === 0} className="w-20 h-1.5 cursor-pointer disabled:opacity-50" />
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input type="color" value={strokeColor} onChange={handleStrokeColorChange} disabled={selectedIds.length === 0} className="w-7 h-7 rounded border border-gray-300 cursor-pointer disabled:opacity-50" />
                    <input type="range" min="0" max="1" step="0.01" value={strokeOpacity} onChange={handleStrokeOpacityChange} disabled={selectedIds.length === 0} className="w-20 h-1.5 cursor-pointer disabled:opacity-50" />
                    <span className="text-xs text-gray-500 w-9">{Math.round(strokeOpacity * 100)}%</span>
                </div>

                {selectedIds.length > 1 && (
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">{selectedIds.length} shapes selected</span>
                )}
            </div>

            {/* Canvas */}
            <canvas ref={canvasRef} className="rounded-xl shadow-md" style={{ display: 'block' }} />
            ...
            <div className="mt-5 bg-white rounded-2xl px-5 py-3 w-[90%] max-w-3xl shadow-sm border border-gray-100">
                ...
                <div className="flex flex-wrap gap-2">
                    {shapes.map(shape => (
                        ...
                        >
                            <div className="w-4 h-4 rounded-sm" style={{ background: shape.fill, border: `1px solid ${shape.stroke}`, borderRadius: shape.type === 'ellipse' ? '50%' : '2px' }} />
