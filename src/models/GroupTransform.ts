...
export class GroupTransform {
  ...
  move(
    ...
    newLeft = Math.min(
      Math.max(0, newLeft),
      canvasWidth - this.initialBounds.w,
    );
  ...
  resize(
    ...
    newBounds.y = Math.min(
      Math.max(0, newBounds.y),
      canvasHeight - newBounds.h,
    );
    if (newBounds.x + newBounds.w > canvasWidth)
      newBounds.w = canvasWidth - newBounds.x;
    if (newBounds.y + newBounds.h > canvasHeight)
      newBounds.h = canvasHeight - newBounds.y;
